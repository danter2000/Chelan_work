...
if (!is.null(start_date) & !is.null(end_date)) {
  ...
  # Filter data based on unit levels if unit_levels are specified
  if (!is.null(unit_levels)) {
    data <- data %>% filter(unit %in% unit_levels)
  }
  
  # Calculate summary statistics
  summary_stats <- data %>%
    group_by(unit, .data[[group_var]]) %>%
    summarise(mean = mean(.data[[y_var]], na.rm = TRUE),
              ...
              n = n()) %>%
    ungroup() %>%
    arrange(unit, .data[[group_var]])
  
  # Format summary statistics table using kable
  kable(summary_stats, caption = "Summary Statistics", format = "html") %>%
...
server <- function(input, output) {
  ...
  output$plot1 <- renderPlot({
    plot_chelan_loess(chelan_df_long, "timestamp_utc", input$Variable)
  })
